...
class FraudDetector:
        ...
        try:
            async with self._lock:
                ...
                if fraud_score >= self.settings.fraud_score_threshold or velocity_triggered:
                    ...
                    # Calculate final score based on severity
                    if velocity_triggered:
                        # Score varies based on velocity count severity
                        if velocity_count >= 10:
                            velocity_boost = 0.95  # CRITICAL - rapid fire attack
                        elif velocity_count >= 8:
                            velocity_boost = 0.80  # HIGH - significant burst
                        elif velocity_count >= 6:
                            velocity_boost = 0.65  # MEDIUM - moderate velocity
                        else:
                            velocity_boost = 0.55  # LOW-MEDIUM - just above threshold
                        final_score = max(fraud_score, velocity_boost)
                    else:
    ...
    def _rule_based_score(self, features: TransactionFeatures) -> float:
        ...
        Produces varied scores from 0.5 to 0.98 for different fraud types.
        """
        import random
        score = 0.05  # Base score
        
        # Velocity-based scoring (progressive)
        if features.velocity_count >= 10:
            score += 0.65 + random.uniform(0, 0.15)  # 0.70-0.85
        elif features.velocity_count >= 8:
            score += 0.50 + random.uniform(0, 0.10)  # 0.55-0.65
        elif features.velocity_count >= 6:
            score += 0.35 + random.uniform(0, 0.10)  # 0.40-0.50
        elif features.velocity_count >= 4:
            score += 0.20 + random.uniform(0, 0.08)  # 0.25-0.33
        
        # Unusual hours (2-5 AM) - more suspicious
        if 2 <= features.hour_of_day <= 5:
            score += 0.15 + random.uniform(0, 0.10)
        
        # High amount deviation - significant factor
        if features.amount_deviation > 3:
            score += 0.25 + random.uniform(0, 0.15)
        elif features.amount_deviation > 2:
            score += 0.15 + random.uniform(0, 0.10)
        elif features.amount_deviation > 1:
            score += 0.08
        
        # High-risk location - strong indicator
        if features.location_risk > 0.7:
            score += 0.30 + random.uniform(0, 0.15)
        elif features.location_risk > 0.5:
            score += 0.15 + random.uniform(0, 0.10)
        
        # Large normalized amount
        if features.amount_normalized > 0.8:
            score += 0.20 + random.uniform(0, 0.10)
        elif features.amount_normalized > 0.5:
            score += 0.10 + random.uniform(0, 0.05)
        elif features.amount_normalized > 0.3:
            score += 0.05
        
        # Weekend late night bonus
        if features.is_weekend and 0 <= features.hour_of_day <= 4:
            score += 0.08
