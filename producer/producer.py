...
def generate_velocity_attack_burst(card_id: str, count: int = 8) -> list[Transaction]:
    ...
    logger.warning(f"Generated velocity attack burst for card {card_id[-8:]}: {count} transactions")
    return transactions


def generate_transaction(is_fraud: bool = False) -> Transaction:
    """Generate a single transaction."""
    # Select a card from the pool
    card_id = random.choice(card_pool)
    
    # Generate timestamp (current time with slight jitter)
    ...
    return Transaction(
        transaction_id=str(uuid.uuid4()),
        card_id=card_id,
        amount=generate_amount(is_fraud=is_fraud),
        timestamp=timestamp.isoformat() + "Z",
    ...
    try:
        while running:
            ...
            if random.random() < VELOCITY_ATTACK_PROBABILITY:
                ...
                burst = generate_velocity_attack_burst(attack_card, count=random.randint(6, 10))
